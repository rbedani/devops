...
Dynamic user profile management: custom fields with configurable types, persistent storage in SQLite, and PDF CV lifecycle (upload, preview, delete). Serves as the data layer for the auto-application pipeline.
...
#### Scenario: Single file field

- GIVEN the user tries to add a second field of type `file`
- WHEN ADD FIELD is clicked with type `file`
- THEN the system shows "Only one CV file field is allowed" and does NOT add the row
